...
carro.adicionarPessoas = function(numPessoas) {
    ...
    let auxPessoas = this.assentos - this.quantidadePessoas == 1 ? 'pessoa' : 'pessoas';
    let totalPessoas = this.quantidadePessoas + numPessoas;

    if((totalPessoas >= this.assentos) && (this.quantidadePessoas == this.assentos))
        return 'O carro já está lotado!';

    else if(totalPessoas > this.assentos)
        return `Só cabem mais ${this.assentos - this.quantidadePessoas} pessoas!`;

    this.quantidadePessoas += numPessoas;
    return `Já temos ${this.quantidadePessoas} ${auxPessoas} no carro!`;
};
...
carro.obterMarcaeModelo(); // 'Esse carro é um Ford Ka'
